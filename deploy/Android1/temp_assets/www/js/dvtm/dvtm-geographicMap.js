...
(function ()
{
  ...
  {
    DvtGeographicMapRenderer.moveCenter(this, centerX, centerY);
  };
  ...
  {
    DvtGeographicMapRenderer.updateZoomLevel(this, zoomLevel);
  };
  ...
  DvtGeographicMapRenderer.render = function (map, mapCanvas, width, height)
  {
    ...
    // move to somewhere else - load complete or ready function of
    // map api
    map.__dispatchEvent({'type': 'ready'});
  };
  ...
  DvtGeographicMapRenderer.renderOracleMap = function (map, mapCanvas, width, height)
  {
    ...
    if (!map['_instance'] || map['_lastKey'] !== (baseURL + '/' + baseMap))
    {
      ...
      var fireMapBoundsChangeEvent = function ()
      {
        ...
        var bbox = map['_instance'].getMapWindowBBox();
        var center = map['_instance'].getCenter();
        // return immediately if properties not initialized
        if (!bbox || !center)
          return;

        var zoomLevel = map['_instance'].getZoomLevel();

        var callback = function (geom)
        {
          var mbr = geom.getMBR();
          map['_instance'].transformGeom(center, 8307, null, function (centerGeom)
          {
        ...
        map['_instance'].transformGeom(bbox, 8307, null, callback);
      };

      var recenter = function ()
      {
        options.mapOptions.initialZooming = 'none';
        map['center'] = map['_instance'].getCenter();
      ...
      {
        options.mapOptions.initialZooming = 'none';
        options.mapOptions.zoomLevel = '' + map['_instance'].getZoomLevel();
        fireMapBoundsChangeEvent();
      };
      ...
      var clickHandler = function (eventId)
      {
        ...
        var location = map['_instance'].getMouseLocation();
        map['_instance'].transformGeom(location, 8307, null, callback);
      };

      map['_instance'].attachEventListener(MVEvent.RECENTER, recenter);
      map['_instance'].attachEventListener(MVEvent.ZOOM_LEVEL_CHANGE, zoom);

      map['_instance'].attachEventListener(MVEvent.MOUSE_CLICK, function ()
      {
        clickHandler('click')
      });

      map['_instance'].attachEventListener(MVEvent.MOUSE_DOWN, function ()
      {
        clickHandler('mousedown')
      });

      map['_instance'].attachEventListener(MVEvent.MOUSE_UP, function ()
      {
  ...
  DvtGeographicMapRenderer.addPointFOI = function (map, mapview, point, pointId, params)
  {
    ...
    pointFOI.enableEventPropagation(false);
   ...
   */
  DvtGeographicMapRenderer.moveCenter = function(map, centerX, centerY)
  {
    var mapProvider = map.getMapProvider();
    ...
    if (mapProvider === DvtGeographicMap.MAP_PROVIDER_ORACLE)
    {
      ...
      if (mapview)
      {
        var mpoint = MVSdoGeometry.createPoint(newLong, newLat, 8307);
        mapview.setCenter(mpoint);
      }
    }
    else if (mapProvider === DvtGeographicMap.MAP_PROVIDER_GOOGLE)
   ...
   * @param {string} zoomLevel - new zoom level
   */
  DvtGeographicMapRenderer.updateZoomLevel = function(map, zoomLevel)
  {
    var mapProvider = map.getMapProvider();
