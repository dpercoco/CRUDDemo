...
(function()
{
  ...
  ThematicMapRenderer.prototype.loadResources = function(amxNode)
  {
    ThematicMapRenderer.superclass.loadResources.call(this, amxNode);
    // don't load basemap resources when source for custom basemap
    // is defined
    ...
    // check if  basemap manager is properly loaded
    if (typeof DvtBaseMapManager === 'undefined')
    {
      return;
    }

    var baseMap = amxNode.getAttribute('basemap');
    var layers = {};
    // visit all areaLayer nodes and get list of all
    // layer names
    amxNode.visitChildren(new adf.mf.api.amx.VisitContext(),
      function (visitContext, visitedNode)
      {
        // skip nodes other with rendered false or
        // nodes which can't be rendered
        if ((visitedNode.isAttributeDefined('rendered') 
          && adf.mf.api.amx.isValueFalse(visitedNode.getAttribute('rendered')))
          || !visitedNode.isReadyToRender())
        {
          return adf.mf.api.amx.VisitResult['REJECT'];
        }

        var tag = visitedNode.getTag();
        switch (tag.getName())
        {
          case 'areaLayer':
            // add info for current layer
            layers[visitedNode.getAttribute('layer')] = true;
            break;

          case 'pointDataLayer':
            // this visitation only applies when cities layer
            // is not yet loaded
            if (!layers['cities'])
            {
              // check child tags for pointName attribute
              // definition
              var childTags = tag.getChildren() || [];
              for (var i = 0, size = childTags.length; i < size; i++)
              {
                if (childTags[i].getAttribute('pointName'))
                {
                  layers['cities'] = true;
                  break;
                }
          ...
          // there is nothing to do in those nodes
          case 'legend':
          case 'areaDataLayer':
            return adf.mf.api.amx.VisitResult['REJECT'];
        }

        // continue in case of fragment or iterator
        return adf.mf.api.amx.VisitResult['ACCEPT'];
      });

    // load resources for all layers in current basemap
    var that = this;
    Object.keys(layers).forEach(function(layer)
    {
      adf.mf.internal.dvt.loadMapLayerAndResource.call(that, amxNode, baseMap, layer);
    });
  };

  ThematicMapRenderer.prototype.isReadyToRender = function(amxNode)
  {
    ...
    return ThematicMapRenderer.superclass.isReadyToRender.call(this, amxNode);
  };
  ...
  ThematicMapRenderer.prototype.ProcessEvent = function(amxNode, event, component)
  {
    ThematicMapRenderer.superclass.ProcessEvent.call(this, amxNode, event, component);
    var renderer = this;
    ...
    {
      itemNode = renderer.findAmxNode(amxNode, event['item']);
  ...
  ThematicMapRenderer.prototype.GetToolkitClass = function()
  {
    return adf.mf.internal.dvt.util.JSONPath('adf.mf.internal.dvt.toolkit.AmxThematicMap');
  };
  ...
  ThematicMapRenderer.prototype.getElementLocator = function (amxNode, domElement)
  {
    ...
    return componentInstance.getSubIdForDomElement(domElement);
  };
  ...
  /**
    ...
    */ 
  ThematicMapRenderer.prototype.getElementForLocator = function(amxNode, elementLocator)
  {
    var componentInstance = this.GetComponentInstance(null, amxNode);
    if (!componentInstance)
    {
    ...
    return componentInstance.getDomElementForSubId(elementLocator);
  };
