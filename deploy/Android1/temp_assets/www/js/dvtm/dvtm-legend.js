...
(function ()
{
  ...
  StandaloneLegendRenderer.prototype.GetToolkitClass = function()
  {
    return adf.mf.internal.dvt.util.JSONPath('adf.mf.internal.dvt.toolkit.Legend');
  };
  ...
  StandaloneLegendRenderer.prototype.GetPreferredSize = function (simpleNode, amxNode, width, height)
  {
    ...
    if (data && componentInstance && componentInstance.getPreferredSize)
    {
      // find out if there is user defined width and height
      var uh = 0;
      var uw = 0;
      if (simpleNode.style.height)
      {
        uh = DOMUtils.parseStyleSize(simpleNode.style.height) || DOMUtils.parseStyleSize(simpleNode.style.height, true);
      }
      uw = DOMUtils.parseStyleSize(simpleNode.style.width) || DOMUtils.parseStyleSize(simpleNode.style.width, true);
      ...
      if (!uh || !uw)
      {
        ...
        try
        {
          size = componentInstance.getPreferredSize(data, width, height);
        }
        catch (ex)
        {
          // in case of empty data legend is unable to return proper data
          // in this case catch exception and continue
          adf.mf.log.Framework.logp(adf.mf.log.level.FINE, this.getTypeName(), "GetPreferredSize", "Exception: " + ex.message + " (line: " + ex.line + ")");
          adf.mf.log.Framework.logp(adf.mf.log.level.FINE, this.getTypeName(), "GetPreferredSize", "Stack: " + ex.stack);
        }
  ...
  StandaloneLegendRenderer.prototype.ProcessEvent = function(amxNode, event, component)
  {
    StandaloneLegendRenderer.superclass.ProcessEvent.call(this, amxNode, event, component);
  ...
  StandaloneLegendSectionRenderer.prototype.CreateMapper = function (discriminant, items)
  {
    return function (cats)
    {
      ...
      if (cats)
      {
        ...
        if (!colors)
          colors = AttributeGroupManager.getSharedAttribute(discriminant, "palette.indicatorColor");
