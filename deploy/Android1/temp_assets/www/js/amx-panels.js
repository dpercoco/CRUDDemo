...
(function()
{
  ...
  // available facets
  var stretchAreas = ["top", "bottom", "center"];

  /**
   * Function creates amxNodes for top, bottom and center facets
   */
  panelStretchLayout.prototype.createChildrenNodes = function(amxNode)
  {
    amxNode.createStampedChildren(null, stretchAreas, null);
    return adf.mf.api.amx.AmxNodeCreateChildrenNodesResult["HANDLED"];
  };

  /** 
   * Visit only available facets 
   */
  panelStretchLayout.prototype.visitChildren = function (amxNode, visitContext, callback)
  {
    return amxNode.visitStampedChildren(null, stretchAreas, null, visitContext, callback);
  };

  panelStretchLayout.prototype.render = function(amxNode, id)
  {
    var rootElement = document.createElement("div");
    ...
    {
      adf.mf.api.amx.enableScrolling(centerElement);
    }
    else if (scrollPolicy != "none")
    {
      adf.mf.api.amx.enableScrolling(centerElement, true); // legacy scrolling only
    }

    var that = this;
    // for each facet render its components
    for (var i = 0, size = stretchAreas.length; i < size; i++)
    {
      var area = stretchAreas[i];
      // get only renderable components
      var facetChildren = amxNode.getRenderedChildren(area);
      if (facetChildren.length > 0)
      {
        // find element for current facet which will be used
        // as a parent element for rendered children
        var areaComponent = that._getAreaElement(rootElement, area);
        if (areaComponent)
        {
          for (var ii = 0, size2 = facetChildren.length; ii < size2; ii++)
          {
            var facetChild = facetChildren[ii].render();
            if (facetChild)
            {
   ...
   */
  panelStretchLayout.prototype.attributeChangeResult = function(amxNode, attributeName, attributeChanges)
  {
    switch (attributeName)
    {
      case "scrollPolicy":
        return adf.mf.api.amx.AmxNodeChangeResult["REFRESH"];
      default:
        return panelStretchLayout.superclass.attributeChangeResult.call(this,
          amxNode, attributeName, attributeChanges);
    }
  };
   ...
   */
  panelStretchLayout.prototype.getDescendentChangeAction = function (amxNode, changes)
  {
    return adf.mf.api.amx.AmxNodeChangeResult["REFRESH"];
  };

  panelStretchLayout.prototype.refresh = function(amxNode, attributeChanges, descendentChanges)
  {
    // handle inline style and styleclass in superclass
    panelStretchLayout.superclass.refresh.call(this, amxNode, attributeChanges, descendentChanges);

    var rootElement = document.getElementById(amxNode.getId());
    if (attributeChanges.hasChanged("scrollPolicy"))
    {
      var scrollPolicy = amxNode.getAttribute("scrollPolicy");
      if (scrollPolicy == "scroll")
      {
      ...
      {
        adf.mf.api.amx.disableScrolling(centerElement);
      }
    }

    if (descendentChanges)
    {
      // process descendent changes
      var that = this;
      // find changed amxNodes for each facet
      for (var i = 0, size = stretchAreas.length; i < size; i++)
      {
        var area = stretchAreas[i];
        var areaComponent = that._getAreaElement(rootElement, area);
        ...
        var nodeMap = [];
        var children = amxNode.getChildren(area);
        for (var ii = 0, size2 = children.length; ii < size2; ii++)
        {
          nodeMap[ii] = document.getElementById(children[ii].getId());
        }

        // find nodes only relevant for this area and render or remove them
        var affectedNodes = descendentChanges.getAffectedNodes();
        for (var iii = 0, size3 = affectedNodes.length; iii < size3; iii++)
        {
          var affectedNode = affectedNodes[iii];
          if (children.indexOf(affectedNode) === -1)
          {
            continue;
          }
            ...
            || changes.hasChanged("rendered"))
          {
            ...
            if (!adf.mf.api.amx.isValueFalse(rendered))
            {
              ...
              var newElement = affectedNode.render();
              ...
              // if the next sibling node is present then insert new one before
              if (nodeMap[index])
              {
                // we have to keep original order to prevent unexpected behavior
                areaComponent.insertBefore(newElement, nodeMap[index]);
              }
              // this is last node visible so only append it as a child of the root element
              else
              {
                areaComponent.appendChild(newElement);
              }
            }
            else
            {
  ...
  /**
   * @param {HTMLElement} rootElement root node of the panelStretchLayout
   * @param {string} area name of the facet for which dom element should be found
   * 
   * @retun {HTMLElement} dom node for area facet or null if no such element is found
   */
  panelStretchLayout.prototype._getAreaElement = function(rootElement, area)
  {
