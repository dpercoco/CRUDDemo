  ...
  ResizeHandler.prototype._registerResizeHandler = function ()
  {
    var resizeHandler = function (event)
    {
      ...
      { 
        ...
        {
          adf.mf.log.Framework.logp(adf.mf.log.level.SEVERE, this.getTypeName(), "callback", "Exception (line: " + exception.line + ")");
      ...
      for(var j = 0; j < postCallbacks.length; j = j + 2)
      {
        ...
        {
          adf.mf.log.Framework.logp(adf.mf.log.level.SEVERE, this.getTypeName(), "postCallback", "Exception (line: " + exception.line + ")");
...
{  
  ...
  {   
    ...
    if (fFamily)
    {
      fFamily = fFamily.replace(new RegExp('\"', 'g'), '\'');
...
(function ()
{
  ...
  {
    return amxNode.getAttribute(BaseRenderer.DATA_OBJECT) || null;
  };
...
(function ()
{
  ...
  BaseComponentRenderer.prototype.loadResources = function(amxNode)
  {
    var that = this;    
    var resId = amxNode.getTag().getName();

    if (!this._toolkitResources)
    {
      if (this._toolkitResourcesLoadingQueue)
      {
        this._toolkitResourcesLoadingQueue.push(amxNode);
        return;
      }
      else
      {
        this._toolkitResourcesLoadingQueue = [amxNode];
      }

      adf.mf.api.resourceFile.loadJsonFile('js/dvtm/json/toolkit-resources.json', true,
        function(result)
        {
          if (!result)
          {
          ...
          result.forEach(function(resource)
          {
            ...
            match.forEach(function(exp)
            {
              if (!(new RegExp(exp)).test(resId))
              {
                hasHit = false;
              }
            });
            ...
            if (hasHit)
            {
              var jsFiles = resource['js'];
              if (jsFiles)
              {
                jsFiles.forEach(function(js)
                {
                  that._toolkitResources['js'].push(js);
                });
              }
            }
          });
          ...
          that._toolkitResourcesLoadingQueue.forEach(function(node)
          {
            args.setAffectedAttribute(node, "_toolkitResLoaded");
          });
          delete that._toolkitResourcesLoadingQueue;

          adf.mf.api.amx.markNodeForUpdate(args);
        },
        function(error)
        {
          that._toolkitResources = null;
          // TODO console
        });
      return;
    }

    BaseComponentRenderer._loaded = BaseComponentRenderer._loaded || {};

    var resourceFiles = this.filterArray(this._toolkitResources['js'], function(file)
    {
      return BaseComponentRenderer._loaded[file] !== true;
    });
    ...
    // concatenated response so we are able to use only one
    // base.js loader for one component
    var toolkitBaseFile = "";

    var load = function(list, toolkit, complete, error)
    {
      if (list.length === 0)
      {
        complete();
      ...
      adf.mf.api.resourceFile._loadFileWithAjax(nextFile, true, function(responseText)
      {
        // base file has to be loaded every time
        if (nextFile === 'js/toolkit/base.js')
        {
          // content of the base file
          var fnBase = responseText;

          // inject function for toolkit file loading
          fnBase += '\nthis.evaluate = function(dvt, _MAFcallback, text) { new Function(\'dvt\', \'_MAFcallback\', text)(dvt, _MAFcallback); }';

          // create loader class
          var fn = new Function(fnBase);

          // create instance of the toolkit resolver
          BaseComponentRenderer._toolkitResolver = new fn();

          // toolkit resolver is ready load next file
          load(list, toolkit, complete, error);
        }
        // prevent evaluation of already evaluated file
        else if (BaseComponentRenderer._loaded[nextFile] !== true)
        {
          BaseComponentRenderer._loaded[nextFile] = true;

          // add final callback statement which returns real toolkit
          responseText = responseText + "\n_MAFcallback(dvt);";

          BaseComponentRenderer._toolkitResolver.evaluate(toolkit.value, function(dvt)
          {
            toolkit.value = dvt;

            // load next file when toolkit is updated
            load(list, toolkit, complete, error);
          },
          responseText);
        }
        ...
        {
          // fall back when prev file has been already loaded
          load(list, toolkit, complete, error);
        }
      },
      function(err) 
      {
        // erase loaded flag and try again later
        BaseComponentRenderer._loaded[nextFile] = false;
    ...
    // file which helps to load minified toolkit resources
    if (!BaseComponentRenderer._toolkitResolver)
    {
      resourceFiles.unshift('js/toolkit/base.js');
    }

    load(resourceFiles, new JSONPath('adf.mf.internal.dvt.toolkit'),
      function()
      {
        var args = new adf.mf.api.amx.AmxNodeUpdateArguments();
        that._loading.forEach(function(node)
        {
          args.setAffectedAttribute(node, "_toolkitLoaded");
        });
      ...
      function(file, err)
      {
        adf.mf.log.Framework.logp(adf.mf.log.level.FINE, self.getTypeName(), "loadResources", 'Toolkit file[' + file + '] failed to load properly.');
      });
  };
  ...
  BaseComponentRenderer.prototype.render = function (amxNode, id)
  {
    ...
    this.loadResources(amxNode);

    // new fresh component so release old toolkit instance if any
    amxNode.setAttributeResolvedValue(COMPONENT_INSTANCE, null);

    // create new object with default and custom styles
    var template = {};

    // initialize template 
    this.InitComponentOptions(amxNode, template);

    // merge default styles
    template = this.MergeComponentOptions(amxNode, template);

    amxNode.setAttributeResolvedValue('__dvtOptionsTemplate', template);
   ...
   *
   * Render is skipped when isReadyToRender function returns false which indicates that some exception occures before
  ...
  {
    if (this.IsAncestor(document.body, node))
    {
      this.GetComponentDimensions(node, amxNode);
    }

    if (this.isReadyToRender(amxNode))
    {
      // component is not waiting on anything
      var args = new adf.mf.api.amx.AmxNodeUpdateArguments();
      // re-schedule the actual rendering for later to move relatively expensive
      // component rendering out of the page display cycle
      args.setAffectedAttribute(amxNode, "_renderMeLater");
      adf.mf.api.amx.markNodeForUpdate(args);
    }
  };

  BaseComponentRenderer.prototype.updateChildren = function (amxNode, attributeChanges)
  {
    return adf.mf.api.amx.AmxNodeChangeResult["REFRESH"];;
  };

  BaseComponentRenderer.prototype.getDescendentChangeAction = function (amxNode, changes)
  {
    var affectedNodes = changes.getAffectedNodes();
    for (var i = 0, size = affectedNodes.length; i < size; i++)
    {
      var node = affectedNodes[i];
      // skip nodes with rendered attribute false
      if (node.isAttributeDefined("rendered") 
          && adf.mf.api.amx.isValueFalse(node.getAttribute('rendered')))
      {
        continue;
      }
      // if node is not ready to render don't do
      // anything and wait for next refresh call
  ...
  BaseComponentRenderer.prototype.refresh = function (amxNode, attributeChanges, descendentChanges)
  {
    ...
    // call tag handler implementation of refresh method to set new
    // inlineStyle and styleClass in case that those are changed
    BaseComponentRenderer.superclass.refresh.call(this, amxNode, attributeChanges);

    if (attributeChanges.hasChanged('inlineStyle'))
    {
      var node = document.getElementById(this.GetComponentId(amxNode));
      this.ResetComponentDimensions(node, amxNode);

      // TODO remove when Bug 24464235 is fixed
      // (sometimes amx is not able to reload inlineStyle properly)
      var style = amxNode.getAttribute('inlineStyle');
      if (style)
      {
        node.setAttribute('style', style);
      }
    ...
    if (attributeChanges.hasChanged('styleClass'))
    {
      this.ResetComponentDimensions(node, amxNode);
    }
    ...
    if (this.isReadyToRender(amxNode))
    {
      var template = amxNode.getAttribute('__dvtOptionsTemplate');
      // clone template to get fresh object
      var options = adf.mf.internal.dvt.util.JSONUtils.cloneObject(template || {});
...
/**
  ...
  */ 
  ...
  BaseComponentRenderer.prototype.SetupComponent = function (amxNode)
  {
    ...
    // write style class common for all dvtm components
    DOMUtils.writeClassAttribute(contentDiv, 'dvtm-component');
    ...
    placeholder.className = "dvtm-component-placeholder";
  ...
  BaseComponentRenderer.prototype._renderComponent = function (simpleNode, amxNode)
  {
    ...
    try
    {
      ...
      simpleNode = document.getElementById(id);
      if (!simpleNode)
      {
        // simpleNode is not in DOM, do not render
        return;
      ...
      // placeholder should be visible only for the first render
      var placeholder = document.getElementById(id + "_placeholder");
      if (placeholder)
      {
        simpleNode.removeChild(placeholder);
      }

      if (amxNode.getAttribute('___dvtRenderingInProgress') === true)
      {
        amxNode.setAttributeResolvedValue('___dvtReRenderRequested', true);
        return;
      }

      amxNode.setAttributeResolvedValue('___dvtRenderingInProgress', true);
      ...
      // render only in case that componentInstance is ready
      if (componentInstance)
      {
  ...
  BaseComponentRenderer.prototype.CreateRenderingContext = function (root, stageId)
  {
    ...
    if (JSONPath('adf.mf.internal.dvt.toolkit.Context'))
    {
      return new adf.mf.internal.dvt.toolkit.Context(root, root.id);
    }
  ...
  BaseComponentRenderer.prototype.CreateComponentCallback = function(amxNode)
  {
    ...
    return {
      "callback": function(event, component)
      {
        if (event['type'] === 'ready')
        {
          amxNode.setAttributeResolvedValue('___dvtRenderingInProgress', null);
          // wait until component is rendered and render it again
          if (amxNode.getAttribute('___dvtReRenderRequested') === true)
          {
            amxNode.setAttributeResolvedValue('___dvtReRenderRequested', null);
          ...
          {
            adf.mf.log.Framework.logp(adf.mf.log.level.FINE, renderer.getTypeName(), "CreateComponentCallback", "Component " + amxNode.getId() + " is ready.");
            // set flag that indicates everything is rendered
            if (amx && amx.testmode === true)
            {
        ...
        // pass event to processing method
        renderer.ProcessEvent(amxNode, event, component);
      }
    };
  };

  BaseComponentRenderer.prototype.ProcessEvent = function(amxNode, event, component)
  {
  ...
  BaseComponentRenderer.prototype.CreateComponentInstance = function (simpleNode, amxNode)
  {
    ...
    var instance = null;
    var clazz = this.GetToolkitClass();
    if (clazz)
    {
      instance = clazz.newInstance(context, callback, callbackObj);
      if (instance)
      {
  ...
  /**
   * 
   * @return class of the toolkit representation of this component.
   */
  BaseComponentRenderer.prototype.GetToolkitClass = function ()
  {
    return null;
  };
   ...
   *
   * @param instance toolkit component instance
   * @param width width of the component
   * @param height height of the component
   * @param amxNode amxNode of this component
   */
  ...
  BaseComponentRenderer.prototype.DestroyComponent = function (simpleNode, amxNode)
  {
    ...
    var instance = amxNode.getAttribute(COMPONENT_INSTANCE);
    if (adf.mf.internal.amx.implementsFunction(instance, 'destroy'))
    {
      instance.destroy();
    }
    amxNode.setAttributeResolvedValue(COMPONENT_INSTANCE, null);
  ...
  BaseComponentRenderer.prototype.isReadyToRender = function (amxNode)
  {
    if (this._loading || this._toolkitResourcesLoadingQueue || amxNode.getAttribute("_rbLoaded") === "loading")
    {
      return false;
    }
  ...
  BaseComponentRenderer.prototype._loadResourceBundles = function (amxNode)
  {
    ...
    if (!adf.mf.environment.profile.dtMode && JSONPath('adf.mf.internal.dvt.toolkit.Bundle') != null)
    {
      if (this._loading)
      {
        this._loading.push(amxNode);
...
(function ()
{
  ...
  DataStampRenderer.prototype.createChildrenNodes = function (amxNode)
  {
    ...
    var action = this.CreateSimpleChildrenNodes(amxNode);
    if (action !== null)
    {
      return action;
    }
  ...
  DataStampRenderer.prototype.updateChildren = function (amxNode, attributeChanges)
  {
    ...
    if (attributeChanges.hasChanged('value'))
    {
      ...
      if (amxNode.getAttribute('_waitForData') !== true)
      {
        ...
        if (dataItems.treeNodeBindings)
        {
          amxNode.setAttributeResolvedValue('_oldCollectionKeys', dataItems.treeNodeBindings.keys);
        }
      }
  ...
  DataStampRenderer.prototype.isReadyToRender = function (amxNode)
  {
    if (amxNode.isAttributeDefined('value'))
    {
      var model = amxNode.getAttribute('value');
      if (model === undefined)
      {
        return false;
      }
      ...
      if (model)
      {
        var iter = adf.mf.api.amx.createIterator(model);
        if (iter.getTotalCount() > iter.getAvailableCount())
        {
          return false;
        }
    ...
    return DataStampRenderer.superclass.isReadyToRender.call(this, amxNode);
  };
  ...
  DataStampRenderer.prototype._nonItemizedUpdate = function (amxNode, facets, dataItems, oldValue)
  {
    ...
    if (oldValue)
    {
      for (i = 0, length = facets.length; i < length; i++)
      {
        ...
        if (iter.isTreeNodeIterator())
        {
          keys = amxNode.getAttribute('_oldCollectionKeys');
...
(function()
{
  ...
  ResourceBundle.createLocalizationBundle = function(resourceName, path, bundleProperty)
  {
    ...
    {
      adf.mf.internal.dvt.toolkit.Bundle.addLocalizedStrings(window[bundleProperty]);  
    };
...
(function ()
{
  ...
  DataLayerRenderer.prototype._updateDataLayerChildrenNodes = function (amxNode, attributeChanges)
  {
    if (attributeChanges.hasChanged("value"))
    {
      ...
      var oldKeys = amxNode.getAttribute('_oldCollectionKeys') || null;
      // create the new stamped children hierarchy
      var dataItems = amxNode.getAttribute("value");
      if (dataItems)
      {
      ...
      if (oldKeys)
      {
        ...
        {
          children = amxNode.getChildren(null, key);
          for (j = children.length - 1; j >= 0; j--)
          {
            amxNode.removeChild(children[j]);
          }
