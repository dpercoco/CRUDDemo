...
(function() {
  ...
  NBoxMarkerRenderer.prototype.ProcessAttributes = function(options, amxNode, context)
  {
    ...
    if (facetName == "indicator")
      facetName = "indicatorIcon";
    var currentNode = options ['_currentNode'];
    var marker = currentNode [facetName];
    if (!marker) 
    {
      marker = {};
      currentNode [facetName] = marker;
    }
    ...
    var groupBy = options._groupBy;
    var groupCategory = currentNode.groupCategory;
    ...
    for (var i = 0; i < attributeChildren.length; i++) 
    {
      ...
      if (rendered) {
        ...
        var groupId = ag.getAttribute("id");
        if (groupBy [groupId]) 
        {
          groupCategory += groupId + ":" + ag.getAttribute("value") + ";";
        }
      }
    }
    ...
    if (groupCategory) 
    {
      currentNode.groupCategory = groupCategory;
    }
    return true;
  };
})();
...
(function() {
  ...
  NBoxNodeRenderer.prototype.ProcessArrayItem = function(options, nodeNode, context)
  {
    ...
    if (groupCategory)
      node.groupCategory = groupCategory;
...
(function ()
{
  ...
  NBoxRenderer.prototype.GetAttributesDefinition = function ()
  {
    ...
    attrs['groupBy'] = 
    {
      'path' : '_groupBy', 'type' : PROP_TEXT_ARRAY
    };
    attrs['groupBehavior'] = 
    {
  ...
  NBoxRenderer.prototype.ProcessEvent = function(amxNode, event, component)
  {
    NBoxRenderer.superclass.ProcessEvent.call(this, amxNode, event, component);
    var renderer = this;
    ...
    if (type === 'selection')
    {
      ...
      var selectedRowKeys = renderer.findAllAmxNodes(amxNode, selection).map(rkMapper);
  ...
  NBoxRenderer.prototype.GetToolkitClass = function()
  {
    return adf.mf.internal.dvt.util.JSONPath('adf.mf.internal.dvt.toolkit.NBox');
  };
  ...
  NBoxRenderer.prototype.RenderComponent = function (instance, width, height, amxNode)
  {
    ...
    for (var i = 0; i < data.nodes.length; i++) 
    {
      ...
      if (node.indicatorIcon && !node.indicatorIcon.shape)
        delete node.indicatorIcon;
    }
    delete data._groupBy;
