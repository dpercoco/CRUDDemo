...
(function()
{
        ...
        function()
        {
          ...
          if (converter != null)
          {
            ...
            if (rawValue === "" && value !== "")
            {
              ...
              // TODO consider using:
              //  adf.mf.api.amx.addMessage("severe", "Unable to convert: "+value, null, null);
              //  but not if there's already another error message displayed.
              //  Sometimes there will be but not always.
              return;
            }
  ...
  function _createInlineCapableImage(amxNode, fetchAndInline, shortDesc, attributeName, rawSource, relativeSource)
  {
      ...
      var failLoad1 = function()
        {
          if (domNode && domNode.setAttribute) // element still exists
          {
            ...
            var altSrcPath = "file://" + relativeSource;
    ...
    {
      adf.mf.internal.amx.getImageDataUriIfNeeded(relativeSource).then(
        function(urlOrDataUri)
        {
          domNode.setAttribute("src", urlOrDataUri);
        ...
        function(failureReason)
        {
          if (adf.mf.log.Framework.isLoggable(adf.mf.log.level.FINE))
          {
            adf.mf.log.Framework.logp(adf.mf.log.level.FINE,
              "amx-commonTags",
              "_createInlineCapableImage",
              "failed to load: \"" + relativeSource + "\" reason: " + failureReason);
          }
          _handleNonInlineSvgError({"target": domNode, "data": amxNode});
        });
  ...
  function _refreshNonInlineImage(amxNode, existingImageElem, rawSource, relativeSource)
  {
    ...
    adf.mf.internal.amx.getImageDataUriIfNeeded(relativeSource).then(
      function(urlOrDataUri)
      {
        existingImageElem.setAttribute("src", urlOrDataUri);
      },
      function(failureReason)
      {
        if (adf.mf.log.Framework.isLoggable(adf.mf.log.level.FINE))
...
// TODO need to add protections if this callback invoked after the component has been removed or refreshed
  ...
  /**
   * Makes the given URL non-relative (eliminates "../" and "./").
   * @param {String} url the URL (without any query string)
   * @param {String} baseUrlWithTrailingSlash undefined or the base URL if known (must have a trailing slash, no query string)
   * @return {String} the non-relative URL
   */
  adf.mf.internal.amx._makeNonRelativeUrl = function(url, baseUrlWithTrailingSlash)
  {
    // Change all multiple consecutive "/" to just a single "/"
    url = url.replace(/[\/]{2,}/g, "/");
    ...
    // Prepend the baseUrlWithTrailingSlash if known
    if (baseUrlWithTrailingSlash && url.charAt(0) != "/")
      url = baseUrlWithTrailingSlash + url;

    // Resolve all the "../" entries (if possible)
    var slashDotDotSlashIndex = -1;
    while ((slashDotDotSlashIndex = url.lastIndexOf("/../")) != -1)
    {
      var slashBefore = url.lastIndexOf("/", slashDotDotSlashIndex - 1);
      if (slashBefore == -1)
        break;
      url = url.substring(0, slashBefore + 1) + url.substring(slashDotDotSlashIndex + 4);
    }
  ...
  /**
   * Test whether the URL is local and not contained within the container's "active"
   * directory.
   * @param {String} url the URL
   * @return {Boolean} false if the URL is non-local or outside the active directory; true otherwise
   */
  adf.mf.internal.amx._localAndNotContainedInTheActiveDirectory = function(url)
  {
    if (!url)
      return false; // no URL provided
    if (amx.hasProtocol(url) &&
      url.indexOf("file://") == -1 &&
      url.indexOf("ms-appdata:///local/") == -1)
      return false; // not local

    var activeDirectory = adf.mf.internal.amx.activeDirectory;
    if (activeDirectory == null)
    {
      if (adf.mf.internal.amx.agent["type"] == "UWP")
      {
        // Get it from a special UWP variable:
        activeDirectory = document.location.href.split('?')[0] + "/../";
        activeDirectory = adf.mf.internal.amx._makeNonRelativeUrl(activeDirectory);
      }
      else
      {
        // Get it from the bootstrap.html query string:
        activeDirectory =
          adf.mf.api.getQueryStringParamValue(adf.mf.api.getQueryString(), "profile_json_folder_override", "") +
          "/../";
        activeDirectory = adf.mf.internal.amx._makeNonRelativeUrl(activeDirectory);
      }

      // Ensure the active directory ends with a slash:
      if (!activeDirectory.endsWith("/"))
        activeDirectory += "/";

      // Strip off the file protocol (if present)
      activeDirectory = activeDirectory.replace("file://", "");

      // Save the value so we don't have to re-compute it:
      adf.mf.internal.amx.activeDirectory = activeDirectory;
    }

    url = adf.mf.internal.amx._makeNonRelativeUrl(url, activeDirectory);

    // Strip off the file protocol (if present)
    url = url.replace("file://", "");

    return url.indexOf(activeDirectory) == 0; // false = not local; true = local
  };

  function _textIndexIsBetween(theString, theSearchText, startIndex, endIndex)
  {
    var foundIndex = theString.indexOf(theSearchText);
    if (foundIndex >= startIndex && foundIndex <= endIndex)
      return true;
    return false;
  }

  // Limit the number of images that we cache so not too much memory is used:
  adf.mf.internal.amx._imageDataUriCache = new adf.mf.internal.BaseLRUCache(50);

  adf.mf.internal.amx.getIosImageDataUriPromise = function(url)
  {
    // This function is only for local problematic URLs for WKWebView on iOS
    return new adf.mf.internal.BasePromise(
      function(resolve, reject)
      {
        var cachedDataUri = adf.mf.internal.amx._imageDataUriCache.get(url);
        if (cachedDataUri != null)
        {
          resolve(cachedDataUri);
        }
        else // not cached
        {
          // Attempt to load the file via WKWebView messaging
          adf.mf.api.resourceFile._loadFileWithAjax(
            url,
            true,
            function(imageData)
            {
              try
              {
                ...
                var lowerCaseDataType = "";
                if (lastDotIndex == -1)
                {
                  // Try to guess the MIME Type from the binary data:
                  if (_textIndexIsBetween(imageData, "xml", 0, 10))
                    lowerCaseDataType = "svg";
                  else if (_textIndexIsBetween(imageData, "GIF", 0, 10))
                    lowerCaseDataType = "gif";
                  else if (_textIndexIsBetween(imageData, "PNG", 0, 10))
                    lowerCaseDataType = "png";
                  else if (_textIndexIsBetween(imageData, "MM", 0, 4))
                    lowerCaseDataType = "tiff";
                  else if (_textIndexIsBetween(imageData, "JFIF", 0, 10))
                    lowerCaseDataType = "jpeg";
                }
                else
                {
                  // Use the extension that was provided on the URL:
                  lowerCaseDataType = url.substring(lastDotIndex + 1).toLowerCase();
                }
                ...
                {
                  case "svg":
                    dataUri = "data:image/svg+xml;" + imageData;
                    break;
                  default:
                    switch (lowerCaseDataType)
                    {
                    ...
                    if (lowerCaseDataType == "")
                      prefix = "data:image";
                    dataUri =
                      prefix + lowerCaseDataType + ";base64," + window.btoa(imageData);
                }
                adf.mf.internal.amx._imageDataUriCache.put(url, dataUri);
                resolve(dataUri);
              }
              catch (problem)
              {
  ...
  adf.mf.internal.amx.getUwpImageDataUriPromise = function(url)
  {
    // This function is only for local problematic URLs for WKWebView on iOS
    return new adf.mf.internal.BasePromise(
      function(resolve, reject)
      {
        ...
        {
          // Via UWP/MafTemplate/MafTemplate/www/js/windows/maf.windows.AdfmfCallback.js
          AdfmfCallback.getDataURIForLocalResource(url).then(
            function(dataUri)
            {
              adf.mf.internal.amx._imageDataUriCache.put(url, dataUri);
              resolve(dataUri);
  ...
  /**
   * Converts the given URL to a data URI if the URL is outside of the container's
   * "active" directory.
   * @param {String} url the URL
   * @return {Promise} a promise that resolves with the URL or a data URI when available
   */
  adf.mf.internal.amx.getImageDataUriIfNeeded = function(url)
  {
    if (!adf.mf.environment.profile.dtMode)
    {
      // Some images will fail to load on devices if they are outside of a specific
      // directory. For example with WKWebView:
      //   Blocked URL:
      //     file:///var/mobile/Containers/Data/Application/0812CB83-5518-4942-801D-63CE932979A9/Documents/e-156.png
      //   Allowed URL:
      //     ../FARs/ViewController/public_html/images/avatar_Karen_144@2x.png
      switch (adf.mf.internal.amx.agent["type"])
      {
        case "iOS":
          if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.loadFileWithAjax)
          {
            // WKWebView, not UIWebView...
            if (adf.mf.internal.amx._localAndNotContainedInTheActiveDirectory(url))
              return adf.mf.internal.amx.getIosImageDataUriPromise(url);
          }
          break;
        case "UWP":
          if (adf.mf.internal.amx._localAndNotContainedInTheActiveDirectory(url))
            return adf.mf.internal.amx.getUwpImageDataUriPromise(url);
          break;
      }
    }

    // fake out the promise here so that we don't have to wait for the callback delay
  ...
  commandLink.prototype.render = function(amxNode)
  {
    ...
    amxApi.addBubbleEventListener(domNode, "click", preventDefaultBehavior);

    if (amxApi.isValueTrue(amxNode.getAttribute("disabled")))
    {
    ...
    else if (amxApi.isValueTrue(amxNode.getAttribute("readOnly")))
    {
    ...
    amxApi.addBubbleEventListener(domNode, "tap", function(event)
      {
        ...
        if (!amxApi.isValueTrue(amxNode.getAttribute("disabled")) &&
          !amxApi.isValueTrue(amxNode.getAttribute("readOnly")))
        {
          amxApi.validate(domNode, function()
            {
              if (amxApi.acceptEvent())
              {
                var event = new amxApi.ActionEvent();
                amxApi.processAmxEvent(amxNode, "action", undefined, undefined, event,
                  function()
                  {
                    var action = amxNode.getAttributeExpression("action", true);
                    if (action != null)
                    {
                      amxApi.doNavigation(action);
                    }
                  });
              }
            });
    ...
    // Prevent browser default drag and drop on anchors. Otherwise it messes with swipe (esp. UWP) 
    amxApi.addBubbleEventListener(domNode, "dragstart", preventDefaultBehavior);

    amxApi.enableAmxEvent(amxNode, domNode, "swipe");
    amxApi.enableAmxEvent(amxNode, domNode, "tapHold");
