...
(function ()
{
  ...
  masonryLayout.prototype.initMLC = function (amxNode, _masonryLayout)
  {
    ...
    callbackInfo.layoutCycleOnEndFunc = function ()
    {
      if (_masonryLayout._locked) {
        ...
        if (_masonryLayout._refreshWhenUnlock)
        {
          var args = new adf.mf.api.amx.AmxNodeUpdateArguments();
          args.setAffectedAttribute(amxNode, "_refreshAfterUnlock");
          adf.mf.api.amx.markNodeForUpdate(args);
          _masonryLayout._refreshWhenUnlock = false;
        }
      }
      var components = _masonryLayout.getElementsByClassName("dvtm-component");
      window.removeEventListener('resize', resizeListener);
      for (var i = 0; i < components.length; i++)
        adf.mf.api.amx.triggerBubbleEventListener(components[i], "resize");
   ...
   * @returns {undefined}
   */
  function getBeforeNode (amxNode) {
    var parentAmxNode = amxNode.getParent ();
    var children = parentAmxNode.getChildren ();
    var after = false;
    for (var i = 0; i < children.length; i++) {
      if (after)
      {
        var afterNode = document.getElementById(children[i].getId());
        if (afterNode)
          return afterNode;
      } else
      if (amxNode == children [i]) 
      {
        after = true;
      }
    }
  ...
  {
    var tileNode = document.getElementById(amxNode.getId());
    amxNode.getParent().getTypeHandler()._refreshTiles(amxNode.getParent(), descendentChanges);
    if (descendentChanges)
    {
      descendentChanges.getAffectedNodes().forEach(function(node)
      {
        var domNode = document.getElementById(node.getAttribute ("id"));
        if (node.getAttribute("rendered") && !domNode)
        {
          domNode = node.render ();
          tileNode.insertBefore(domNode, getBeforeNode(node));
        } else
        if (!node.getAttribute("rendered") && domNode)
        {
          tileNode.removeChild(domNode);
        }
      });
    }
  };
