      ...
      else
      {
        var fmtNumber =
          TrNumberFormat.getRoundedValue(
            this._roundingMode,
            parseFloat(number),
            this._numberFormat.getMaximumFractionDigits());
  ...
  )
{
  ...
  if(this._isConvertible(numberString))
  {
    ...
    {
      // Divide by 100, since the percentages are internally stored as decimal fractions. Ex: 29% is 0.29
      // The obvious thing to do is divide the number by 100, but doing so will expose floating point precision errors.
      // Ex: 6.56 / 100 = 6.55999999999999
      // Hence convert the number to string form and left shift the decimal point by 2 places, prefixing zeros in the process if required.
      // Convert back to number once the decimal is shifted
      parsedValue = this._numberFormat.moveDecimalLeft(parsedValue);
    }
    return parsedValue;
  }
  else
