...
(function()
{
  ...
  listItem.prototype.render = function(amxNode, id)
  {
    ...
    // We are adding title attribute to the listItemElement for ACC purposes
    if (shortDesc != null)
    {
    ...
    // If there is 'action' attribute set on listItem, set WAI-ARIA role of button to the DOM element, else no role is set
    if (amxNode.getAttributeExpression("action", true) != null)
    {
      listItemElement.setAttribute("role", "button");
    }
    var descendants = amxNode.renderDescendants();
    for (var i = 0, size=descendants.length; i < size; ++i)
    {
