...
(function()
{
  ...
  selectBooleanSwitch.prototype.render = function(amxNode, id)
  {
    ...
    var onLabel = amxNode.getAttribute("onLabel") || 
        adf.mf.resource.getInfoString("AMXInfoBundle", "amx_selectBooleanSwitch_ON_LABEL");
    var offLabel = amxNode.getAttribute("offLabel") || 
        adf.mf.resource.getInfoString("AMXInfoBundle", "amx_selectBooleanSwitch_OFF_LABEL");
        ...
        adf.mf.api.amx.addDragListener(
          switchElement,
          {
            start: function(event, dragExtra)
              {
                var element = this;
                dragExtra.requestDragLock(element, true, false); // horizontal drag lock
              },
    ...
    attributeChanges)
  {
    ...
    if (readOnly)
    {
      ...
      switchElement.textContent = isOn ?
        amx.getTextValue(amxNode.getAttribute("onLabel") || 
            adf.mf.resource.getInfoString("AMXInfoBundle", "amx_selectBooleanSwitch_ON_LABEL")) :
        amx.getTextValue(amxNode.getAttribute("offLabel") || 
            adf.mf.resource.getInfoString("AMXInfoBundle", "amx_selectBooleanSwitch_OFF_LABEL"));
    }
    else
    {
    ...
    attributeChanges)
  {
    ...
    {
      var switchElement = domElement.querySelector(".readOnlyLabel");
      switchElement.textContent = amx.getTextValue(onLabel);
    }
    else if (!readOnly)
    {
    ...
    attributeChanges)
  {
    ...
    {
      var switchElement = domElement.querySelector(".readOnlyLabel");
      switchElement.textContent = amx.getTextValue(offLabel);
    }
    else if (!readOnly)
    {
  ...
  selectBooleanSwitch.prototype._handleDrag = function(amxNode, event, dragExtra)
  {
    ...
    if (!swipeConsumed)
    {
      ...
      if (swipeExtra)
      {
            ...
            (!isOn && swipeType == "swipeEnd"))
        {
          ...
          switchElement.removeAttribute("data-swipeDone");
          this._handleSwitch(amxNode, event);
        }
      }
    }
  };
